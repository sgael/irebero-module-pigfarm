...
(function(window,document,undefined){
	var factory=function($,DataTable){
		var Checkboxes=function(settings){
			...
			if(this.s.dt.settings()[0].checkboxes){return;}
		...
		Checkboxes.prototype={_constructor:function(){
			var self=this;var dt=self.s.dt;
			...
			for(var i=0;i<ctx.aoColumns.length;i++){
				if(ctx.aoColumns[i].checkboxes){
					...
					DataTable.ext.internal._fnApplyColumnDefs(ctx,[{"targets":i,"searchable":false,"orderable":false,"width":"1%","className":"dt-body-center","render":function(data,type,full,meta){
						if(type==="display"){
							data='<input type="checkbox" class="dt-checkboxes">';
						}
							return data;}}],{},function(iCol,oDef){
					...
					if(ctx.aoColumns[i].es.selectRow){
					...
					if(hasCheckboxes){
						...
						$tableBody.on("click","input.dt-checkboxes",function(e){self.onClick(e,this);
						});
						if(hasCheckboxesSelectRow){
							...
							}$table.on("draw.dt",function(e,ctx){self.onDraw(e,ctx);
							});
							$tableContainer.on("click",'thead th.dt-checkboxes-select-all input[type="checkbox"]',function(e){self.onClickSelectAll(e,this);
							});
					...
					if(nodes.length){
						if(DataTable.select){
							...
							}else{dt.$(nodes).removeClass("selected");
						}
					}
				}
				...
				var self=this;var dt=self.s.dt;var ctx=dt.settings()[0];
				...
				var cells=dt.cells("tr",col,{
					page:((ctx.aoColumns[col].checkboxes&&ctx.aoColumns[col].checkboxes.selectAllPages)?"all":"current"),search:"applied"0});
				...
				if(ctx.aoColumns[col].checkboxes.selectRow){
					var rows=dt.rows({page:((ctx.aoColumns[col].checkboxes&&ctx.aoColumns[col].checkboxes.selectAllPages)?"all":"current"),search:"applied"});
					...
					var self=this;var dt=self.s.dt;ctx=dt.settings()[0];
					var rows_seen={};
					dt.cells("tr",self.s.columns,{page:"current",search:"applied"}).every(function(){var cellColIdx=this.index().column;var cellRowIdx=this.index().row;
						var cellData=this.data();
						var index=$.inArray(cellData,ctx.checkboxes.s.data[cellColIdx]);
						if(index!==-1){
							if(!rows_seen.hasOwnProperty(cellRowIdx)){
								self.updateCheckbox("cell",this.node(),true);self.updateSelect("cell",this.node(),true);
					...
					for(var colIdx=0;colIdx<ctx.aoColumns.length;colIdx++){
						if(ctx.aoColumns[colIdx].checkboxes&&ctx.aoColumns[colIdx].checkboxes.selectAll){
							...
							if(ctx._fixedHeader){
								if(ctx._fixedHeader.dom["header"].floating){$checkboxesSelectAll=$('.fixedHeader-floating .dt-checkboxes-select-all[data-col="'+colIdx+'"] input[type="checkbox"]');
							}
						}
				...
				var add=function(name,num){$output.append($('<span class="select-item"/>').append(dt.i18n("select."+name+"s",{_:"%d "+name+"s selected",0:"",1:"1 "+name+" selected"},num)));
			};
			var colSelectRowIdx=-1;
			for(var colIdx=0;colIdx<ctx.aoColumns.length;colIdx++){
				if(ctx.aoColumns[colIdx].checkboxes&&ctx.aoColumns[colIdx].checkboxes.selectRow){
					colSelectRowIdx=colIdx;break;
				}
			}
